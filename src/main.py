...
def analyze_scale(file_scale, do_plot=False):
    ...
    print(f"Linearity Err:   {M['Linearity_%']:.4f} %FS")
    ...
    if do_plot:
        ...
        plt.title("Scale Factor Fit"); plt.legend(); plt.show()
...
def analyze_drift(file_drift, do_plot=False):
    ...
    print(f"Bias Instability:{M['BI']:.6e} °/s")
    ...
    if do_plot:
        ...
        plt.title("Allan Deviation"); plt.show()

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--scale", help="Путь к МК.dat")
    parser.add_argument("--drift", help="Путь к Дрейф.dat")
    parser.add_argument("--plot", action="store_true", help="Показывать графики")
